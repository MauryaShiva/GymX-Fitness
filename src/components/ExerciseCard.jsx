...
const ExerciseCard = ({ exercise }) => {
  return (
    ...
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
      className="relative w-full max-w-[350px] h-[400px] md:h-[450px] mx-auto bg-surface rounded-2xl overflow-hidden shadow-xl group transition-shadow duration-300 ease-in-out hover:shadow-2xl hover:shadow-primary/20"
    >
      <Link to={`/exercise/${exercise.exerciseId}`} className="block w-full h-full">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 text-text-primary">
          ...
          <div className="flex flex-row flex-wrap gap-2 mb-3">
            <span className="bg-primary/80 text-background text-xs font-semibold rounded-full capitalize py-1 px-3 backdrop-blur-md shadow-sm">
              {exercise.bodyParts[0]}
            </span>
            <span className="bg-secondary/80 text-background text-xs font-semibold rounded-full capitalize py-1 px-3 backdrop-blur-md shadow-sm">
