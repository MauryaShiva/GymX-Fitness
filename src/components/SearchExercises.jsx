...
const SearchExercises = ({ onSearch, bodyPart, setBodyPart }) => {
  ...
  const renderSearchContent = (isMobile = false) => (
    <div className="relative w-full max-w-3xl mb-16">
      ...
      {!isMobile && (
        <button
          className="absolute right-2 top-1/2 -translate-y-1/2 bg-gradient-to-r from-red-600 to-red-800 text-white font-bold h-12 px-8 rounded-full text-lg hover:scale-105 active:scale-95 transform transition-all duration-300 shadow-lg shadow-red-500/20 focus:outline-none"
      ...
      {suggestions.length > 0 && (
        <ul className={`absolute mt-2 w-full bg-gray-800 border border-gray-700 rounded-xl shadow-lg z-50 text-left overflow-y-auto max-h-60 ${isMobile ? 'top-full' : 'top-full'}`}>
          {suggestions.map((suggestion, index) => (
            <li
              key={index}
              onClick={() => handleSuggestionSelect(suggestion)}
              className="px-5 py-4 text-gray-300 hover:bg-red-600 hover:text-white cursor-pointer transition-colors duration-200 capitalize border-b border-gray-700 last:border-0 text-lg md:text-base"
            >
              {suggestion}
            </li>
          ))}
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isMobileSearchOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-xl flex flex-col p-4 pt-safe-top md:hidden"
          >
      ...
      <section className="flex flex-col items-center mt-12 p-5 text-center bg-gradient-to-b from-background via-gray-900 to-background text-white py-20">
        ...
        <div className="md:hidden w-full max-w-3xl mb-16">
          <button
            onClick={() => setIsMobileSearchOpen(true)}
            className="w-full h-16 bg-gray-800/50 text-gray-400 border border-gray-700 rounded-full py-2 px-6 text-lg flex items-center gap-4 backdrop-blur-sm"
          >
            <Search className="h-6 w-6" />
