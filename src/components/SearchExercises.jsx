...
const SearchExercises = ({ onSearch, bodyPart, setBodyPart, isOverlayOpen, setIsOverlayOpen }) => {
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    setSuggestions([]);
    onSearch(suggestion);
    setIsOverlayOpen && setIsOverlayOpen(false);
    document
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOverlayOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className="md:hidden fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex flex-col items-center pt-safe px-4"
          >
            ...
            <div className="w-full mt-4 flex-grow">
              <h2 className="text-3xl font-extrabold mb-8 tracking-tighter text-center">
                Search <span className="text-red-500">Workouts</span>
              </h2>
              {searchContent}
            </div>
          </motion.div>
      ...
      {/* Mobile Scrollbar Section (always visible on mobile) */}
      <section className="md:hidden flex flex-col items-center mt-6 px-4">
        <h3 className="text-xl font-bold mb-4 w-full text-left">Categories</h3>
