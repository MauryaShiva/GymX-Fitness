...
const SearchExercises = ({ onSearch, bodyPart, setBodyPart, isMobileSearchOpen, setIsMobileSearchOpen }) => {
  ...
  const handleLocalSearch = () => {
    ...
    if (search && isValidSearch) {
      onSearch(search);
      setSuggestions([]);
      if (setIsMobileSearchOpen) setIsMobileSearchOpen(false); // Close modal on search
      document
        .getElementById("exercises")
        ?.scrollIntoView({ behavior: "smooth" });
    } else {
  ...
  const renderSearchContent = () => (
    <>
      <h2 className="text-3xl md:text-4xl lg:text-6xl font-extrabold mb-8 md:mb-12 tracking-tighter text-text-primary hidden md:block">
      ...
      <div className="relative w-full max-w-3xl mb-8 md:mb-16">
        ...
        <input
          className="w-full h-14 md:h-16 bg-surface text-text-primary placeholder-text-secondary border border-gray-700 rounded-full py-2 pl-14 md:pl-16 pr-24 md:pr-40 text-base md:text-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-300"
          value={search}
        ...
        />
        <button
          className="absolute right-2 top-1/2 -translate-y-1/2 bg-primary text-background font-bold h-10 md:h-12 px-4 md:px-8 rounded-full text-sm md:text-lg hover:bg-primary/90 transition-colors focus:outline-none"
        ...
        {suggestions.length > 0 && (
          <ul className="absolute top-full mt-2 w-full bg-surface border border-gray-700 rounded-xl shadow-2xl z-50 text-left overflow-hidden max-h-60 overflow-y-auto">
            {suggestions.map((suggestion, index) => (
              <li
                key={index}
                onClick={() => handleSuggestionClick(suggestion)}
                className="px-5 py-3 text-text-primary hover:bg-primary/20 cursor-pointer transition-colors duration-200 capitalize border-b border-gray-800 last:border-0"
              >
                {suggestion}
              </li>
            ))}
  ...
  return (
    <>
      {/* Desktop View */}
      <section className="hidden md:flex flex-col items-center mt-12 p-5 text-center py-20 w-full">
        {renderSearchContent()}
      </section>
      ...
      <AnimatePresence>
        {isMobileSearchOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-50 bg-background md:hidden flex flex-col pt-safe-top"
          >
